...
# 52 WEEK HIGH AND LOW UPDATE
def update_52week_stats(conn, type):
    cur = conn.cursor()
    try:
        # -----------------------------------------------------
        # Detect first run
        # -----------------------------------------------------
        if type == 'index':
            cur.execute("SELECT COUNT(*) FROM index_52week_stats")
        else:   
            cur.execute("SELECT COUNT(*) FROM equity_52week_stats")
            
        first_run = cur.fetchone()[0] == 0

        if first_run:
            log("📊 52W STATS: FIRST RUN (full rebuild)")
            if type == 'index':
                cur.execute("""
            ...
            log("📊 52W STATS: INCREMENTAL UPDATE")
            if type == 'index':
                cur.execute("""
                    SELECT DISTINCT index_id
                    FROM index_price_data
        ...
        for symbol_id in symbol_ids:
            try:
                ...
                high_52, low_52 = cur.fetchone()

                if high_52 is None:
                    continue
                if type == 'index':
                    cur.execute("""
                    INSERT INTO index_52week_stats
                    (index_id, week52_high, week52_low, as_of_date)
                    VALUES (?, ?, ?, date('now'))
                ...
                """, (symbol_id, high_52, low_52))
                    ...
                    cur.execute("""
                        INSERT INTO equity_52week_stats
                        (symbol_id, week52_high, week52_low, as_of_date)
                        VALUES (?, ?, ?, date('now'))
                ...
                log(f"⚠️ 52W STATS failed for symbol_id={symbol_id}: {e}")
                continue

        conn.commit()
        log(f"✅ 52W STATS UPDATED: {updated} symbols")
